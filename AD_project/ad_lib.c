...
{        
    	int choice=0,ch1;
	char epass[20],spass[20],rpass[20],ch;
        int n,i=0;
	strcpy(spass,"admin");
	strcpy(rpass,"student");
	...
	if(ch1==1)
	{
        printf("Please confirm your password to proceed.\n");
	scanf("%s", epass);
	n=strcmp(epass,spass);
	if(n==0)
	{
	printf("\npassword is correct");
	printf("\n\n\tWELCOME\n\n");
...
else if(ch1==2)
{
	...
	printf("\n\n\t\tWELCOME\n\n");
	    ...
	    printf("\n\n\t\t<<LIBRARY MANAGEMENT SYSTEM>>(Beta version) \n");
    do{
    printf("\n\t~~MENU~~\n 1> Search a book\n 2> Display All Books of An Author\n 3> List Titles of a Book\n 4> List Count of Books (Issued & On Stock)\n 5> To Issue a Book\n 6> To Rreturn a Book\n 7> Exit the program\n\n\t Enter your choice <1-7>: ");
    scanf("%d",&choice);        
    switch (choice)
    {    
        case 1:
...
void Add_student()
{
    ...
    studentcrd = fopen("student.txt","a+");
    printf("Enter The userid of the student(Integer) :\n");
        scanf("%d",&membr.mid);
...
void bookret()
{
int mid,i,Found1=0,Found2=0,flag=0;char retbookname[20],pass;
    temp1=librecord;temp2=membrrecord;
    printf("\nEnter The userid of the Member :\n");
        scanf("%d",&mid);
    if((membrrecord=fopen("membrrecord.txt","r"))==NULL)
